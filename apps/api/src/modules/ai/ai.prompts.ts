export const PROMPTS: Record<string, { system: string; userTemplate: string }> = {
  unified_intelligence: {
    ...
    userTemplate: `Capability: {{capability}}\nReporting period: {{from}} to {{to}}\nQuestion: {{question}}\nAudience: {{audience}}\nTone: {{tone}}\n\nAuthorised source records:\n{{records}}\n\nProduce a concise, evidence-linked response. If there is insufficient evidence, say so explicitly. For family_communication_draft, produce a draft only for manager review; never state that it was sent. For rota_alternatives, explain each suggested alternative and the constraint it addresses; never claim that a rota was changed or published.`,
  },
  manager_briefing: {
