...
export class OutcomesRepository {
  ...
  static async recordAssessment(data: any) {
    const { scale_id, service_user_id, scores, total_score, band_label, assessed_by, notes } = data;
    const result = await query(
      `INSERT INTO outcome_scale_results (scale_id, service_user_id, scores, total_score, band_label, assessed_by, notes)
       VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`,
      [scale_id, service_user_id, JSON.stringify(scores), total_score, band_label || null, assessed_by || null, notes || null]
    );
    return result.rows[0];
  }

  static async findResults(scaleId: string, filters: { service_user_id?: string; date_from?: string; date_to?: string }) {
    let sql = `
      SELECT osr.*,
        (SELECT first_name || ' ' || last_name FROM service_users WHERE id = osr.service_user_id) AS service_user_name,
    ...
    let idx = 2;
    if (filters.service_user_id) { sql += ` AND osr.service_user_id = $${idx++}`; params.push(filters.service_user_id); }
  ...
  static async findAllResults(orgId: string, filters: { service_user_id?: string }) {
    let sql = `
      SELECT osr.*,
        (SELECT first_name || ' ' || last_name FROM service_users WHERE id = osr.service_user_id) AS service_user_name,
  ...
  static async getServiceUserSummary(serviceUserId: string) {
    const byScale = await query(`
      SELECT os.name, os.shortcode, os.id AS scale_id,
        ...
        (SELECT osr2.band_label FROM outcome_scale_results osr2
         WHERE osr2.scale_id = os.id AND osr2.service_user_id = $1
  ...
  static async getServiceUserTrend(serviceUserId: string, scaleId?: string, days = 90) {
    let sql = `
    ...
    const params: any[] = [serviceUserId, days];
    let idx = 3;
  ...
  static async getOrgSummary(orgId: string) {
    const total = await query(`
      SELECT COUNT(*)::int AS total_assessments,
        COUNT(DISTINCT osr.service_user_id)::int AS service_users_assessed
      FROM outcome_scale_results osr
      JOIN outcome_scales os ON osr.scale_id = os.id
      WHERE os.organization_id = $1
    `, [orgId]);
    ...
    return {
      ...
      service_users_assessed: total.rows[0]?.service_users_assessed ?? 0,
