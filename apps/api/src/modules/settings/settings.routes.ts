...
router.patch('/capabilities', requireRole(UserRole.ORG_ADMIN, UserRole.MANAGER), validate(z.object({ medication_support: z.boolean().optional(), nutrition_support: z.boolean().optional() })), asyncHandler(SettingsController.updateCareCapabilities));
// Areas/locations are day-to-day operational records (the Areas page lets
// managers create and edit them), so managers may manage them too.
router.post('/locations', requireRole(UserRole.ORG_ADMIN, UserRole.MANAGER), validate(createLocationSchema), asyncHandler(SettingsController.createLocation));
router.put('/locations/:id', requireRole(UserRole.ORG_ADMIN, UserRole.MANAGER), validate(updateLocationSchema), asyncHandler(SettingsController.updateLocation));
router.delete('/locations/:id', requireRole(UserRole.ORG_ADMIN, UserRole.MANAGER), asyncHandler(SettingsController.deleteLocation));
...
router.get('/locations/:locationId/carers', asyncHandler(SettingsController.getLocationCarers));
...
router.post('/compliance-config', requireRole(UserRole.ORG_ADMIN, UserRole.MANAGER), validate(createComplianceConfigSchema), asyncHandler(SettingsController.createComplianceConfig));
router.put('/compliance-config/:id', requireRole(UserRole.ORG_ADMIN, UserRole.MANAGER), validate(updateComplianceConfigSchema), asyncHandler(SettingsController.updateComplianceConfig));
...
router.post('/calculate-entitlements', requireRole(UserRole.ORG_ADMIN, UserRole.MANAGER), asyncHandler(SettingsController.calculateStaffEntitlement));
...
router.post('/compliance-profiles', requireRole(UserRole.ORG_ADMIN, UserRole.MANAGER), validate(createComplianceProfileSchema), asyncHandler(SettingsController.createComplianceProfile));
router.put('/compliance-profiles/:id', requireRole(UserRole.ORG_ADMIN, UserRole.MANAGER), validate(updateComplianceProfileSchema), asyncHandler(SettingsController.updateComplianceProfile));
...
router.post('/assign-compliance-profile', requireRole(UserRole.ORG_ADMIN, UserRole.MANAGER), validate(assignComplianceProfileSchema), asyncHandler(SettingsController.assignComplianceProfile));
