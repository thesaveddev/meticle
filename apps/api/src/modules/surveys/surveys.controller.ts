...
export class SurveysController {
  ...
  static async submitSatisfaction(req: Request, res: Response) {
    ...
    const { person_id, respondent_name, relationship, rating, comments } = req.body;
    if (!rating || rating < 1 || rating > 5) {
      throw new AppError(400, 'Rating must be between 1 and 5');
    }
    const survey = await SurveysRepository.createSatisfaction(orgId, {
      person_id, respondent_name, relationship, rating, comments,
    });
  ...
  static async sendSatisfactionInvitation(req: Request, res: Response) {
    const orgId = req.user!.organizationId;
    if (!orgId) throw new AppError(400, 'Organization ID required');
    const { email, person_id, person_name } = req.body;
    if (!email) throw new AppError(400, 'Recipient email is required');
    const inv = await SurveysRepository.createInvitation(orgId, 'satisfaction', email, person_id, person_name);
    ...
    const personText = person_name ? ` for ${escapeHtml(person_name)}` : '';
  ...
  static async publicSubmitSatisfaction(req: Request, res: Response) {
    ...
    const { respondent_name, relationship, rating, comments, person_name } = req.body;
    if (!rating || rating < 1 || rating > 5) {
      throw new AppError(400, 'Rating must be between 1 and 5');
    }
    ...
    // If person_name provided, update the invitation record
    if (person_name && survey) {
      await migrateQuery(`UPDATE survey_invitations SET person_name = $1 WHERE token = $2`, [person_name, token]).catch(logWarn('update survey person_name'));
    }
  ...
  static async publicGetForm(req: Request, res: Response) {
    ...
    if (inv.type === 'engagement' && inv.template_id) {
      const tmplResult = await migrateQuery(`SELECT questions FROM engagement_templates WHERE id = $1`, [inv.template_id]);
