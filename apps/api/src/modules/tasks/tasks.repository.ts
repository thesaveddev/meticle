...
export class TaskRepository {
  static async findAll(orgId: string, filters?: { status?: string; assigned_to?: string; service_user_id?: string }) {
    let sql = `SELECT t.*, COALESCE(sp.first_name || ' ' || sp.last_name, '') as assigned_name,
               COALESCE(su.first_name || ' ' || su.last_name, '') as service_user_name,
        ...
        LEFT JOIN service_users su ON t.service_user_id = su.id
    ...
    if (filters?.service_user_id) { sql += ` AND t.service_user_id = $${idx++}`; params.push(filters.service_user_id); }
  ...
  static async create(orgId: string, data: any) {
    const result = await query(
      `INSERT INTO tasks (organization_id, title, description, assigned_to, service_user_id, priority, status, due_date, created_by)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *`,
      [orgId, data.title, data.description || null, data.assigned_to || null, data.service_user_id || null,
  ...
  static async update(id: string, orgId: string, data: any) {
    ...
    const allowed = new Set(['title', 'description', 'assigned_to', 'service_user_id', 'priority', 'status', 'due_date']);
