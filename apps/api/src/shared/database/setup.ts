...
const MIGRATION_059: Migration = {
  name: '059_email_queue_recovery',
  strict: true,
  statements: (() => {
    const p = path.join(__dirname, 'migrations', '059_email_queue_recovery.sql');
    return [fs.readFileSync(p, 'utf8')];
  })(),
};

const MIGRATION_060: Migration = {
  name: '060_cash_check_two_person_control',
  strict: true,
  statements: (() => {
    const p = path.join(__dirname, 'migrations', '060_cash_check_two_person_control.sql');
    return [fs.readFileSync(p, 'utf8')];
  })(),
};

const MIGRATION_061: Migration = {
  name: '061_cash_reconciliation_two_person',
  strict: true,
  statements: (() => {
    const p = path.join(__dirname, 'migrations', '061_cash_reconciliation_two_person.sql');
    return [fs.readFileSync(p, 'utf8')];
  })(),
};

const MIGRATION_062: Migration = {
  name: '062_cash_check_columns',
  strict: true,
  statements: (() => {
    const p = path.join(__dirname, 'migrations', '062_cash_check_columns.sql');
    return [fs.readFileSync(p, 'utf8')];
  })(),
};
...
const MIGRATION_058: Migration = {
  ...
  statements: [
    `CREATE TABLE IF NOT EXISTS cash_balance_checks (
      ...
      expected_balance_pence INTEGER NOT NULL DEFAULT 0,
      physical_balance_pence INTEGER NOT NULL DEFAULT 0,
      variance_pence INTEGER NOT NULL DEFAULT 0,
      notes TEXT,
...
const MIGRATION_057: Migration = {
  name: '057_expense_void_audit_trail',
  strict: false,
  statements: [
    `ALTER TABLE person_expenses ADD COLUMN IF NOT EXISTS is_voided BOOLEAN NOT NULL DEFAULT FALSE`,
    `ALTER TABLE person_expenses ADD COLUMN IF NOT EXISTS void_reason TEXT`,
    `ALTER TABLE person_expenses ADD COLUMN IF NOT EXISTS voided_by UUID REFERENCES users(id) ON DELETE SET NULL`,
    `ALTER TABLE person_expenses ADD COLUMN IF NOT EXISTS voided_at TIMESTAMPTZ`,
    `ALTER TABLE person_expenses ADD COLUMN IF NOT EXISTS updated_by UUID REFERENCES users(id) ON DELETE SET NULL`,
  ],
};

const MIGRATION_056: Migration = {
  name: '056_migration_checksum_baselines',
  strict: true,
  statements: (() => {
    const p = path.join(__dirname, 'migrations', '056_migration_checksum_baselines.sql');
    return [fs.readFileSync(p, 'utf8')];
  })(),
};
...
const MIGRATION_030: Migration = {
  name: '030_location_max_staff_on_leave',
  strict: false,
  statements: [
    `ALTER TABLE locations ADD COLUMN IF NOT EXISTS max_staff_on_leave INTEGER`,
  ],
};
...
const MIGRATION_041: Migration = {
  name: '041_mission_control_alerts',
  strict: false,
  statements: [
...
const MIGRATION_042: Migration = {
  name: '042_nutrition_module',
  strict: false,
  statements: [
    // Dietary profiles - person-centred dietary information
    ...
    `CREATE INDEX IF NOT EXISTS idx_dietary_profiles_person ON dietary_profiles(person_id)`,
    `CREATE UNIQUE INDEX IF NOT EXISTS idx_dietary_profiles_person_unique ON dietary_profiles(person_id)`,
    // Meal records - what was served and consumed
    ...
    `CREATE INDEX IF NOT EXISTS idx_meal_records_person ON meal_records(person_id)`,
    `CREATE INDEX IF NOT EXISTS idx_meal_records_date ON meal_records(person_id, meal_date)`,
    ...
    // Nutrition alerts - for low intake or concerning patterns
    ...
    `CREATE INDEX IF NOT EXISTS idx_nutrition_alerts_org ON nutrition_alerts(organization_id, dismissed, severity)`,
    `CREATE INDEX IF NOT EXISTS idx_nutrition_alerts_person ON nutrition_alerts(person_id)`
  ],
};

const MIGRATION_043: Migration = {
  name: '043_compliance_portal_tokens',
  strict: false,
  statements: [
    // Compliance officer portal access tokens for location-scoped read-only access
    ...
    `CREATE INDEX IF NOT EXISTS idx_portal_tokens_org ON compliance_portal_tokens(organization_id)`,
    `CREATE INDEX IF NOT EXISTS idx_portal_tokens_location ON compliance_portal_tokens(location_id)`,
    `CREATE INDEX IF NOT EXISTS idx_portal_tokens_email ON compliance_portal_tokens(email)`,
  ],
};
...
const MIGRATION_046: Migration = {
  name: '046_ocr_document_columns',
  strict: false,
  statements: [
    `ALTER TABLE documents ADD COLUMN IF NOT EXISTS ocr_extracted_text TEXT`,
    `ALTER TABLE documents ADD COLUMN IF NOT EXISTS ocr_confidence NUMERIC(5,2)`,
  ],
};

const MIGRATION_047: Migration = {
  name: '047_portal_token_jwt_column',
  strict: false,
  statements: [
    `ALTER TABLE compliance_portal_tokens ADD COLUMN IF NOT EXISTS jwt_token TEXT`,
  ],
};

const MIGRATION_048: Migration = {
  name: '048_email_queue_from_email',
  strict: false,
  statements: [
...
const MIGRATION_050: Migration = {
  name: '050_email_queue_attachments',
  strict: false,
  statements: [
    `ALTER TABLE email_queue ADD COLUMN IF NOT EXISTS attachments JSONB DEFAULT '[]'`,
  ],
};

const MIGRATION_051: Migration = {
  name: '051_invoice_due_date',
  strict: false,
  statements: [
    `ALTER TABLE invoices ADD COLUMN IF NOT EXISTS due_date DATE`,
    `CREATE INDEX IF NOT EXISTS idx_invoices_due_date ON invoices(organization_id, due_date) WHERE status = 'open'`,
  ],
};
...
const MIGRATION_071: Migration = {
  name: '071_org_service_types',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '071_org_service_types.sql'), 'utf8')],
};

const MIGRATION_072: Migration = {
  name: '072_billing_config',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '072_billing_config.sql'), 'utf8')],
};

const MIGRATION_073: Migration = {
  name: '073_homecare_client_billing',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '073_homecare_client_billing.sql'), 'utf8')],
};

const MIGRATION_074: Migration = {
  name: '074_homecare_client_invoicing',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '074_homecare_client_invoicing.sql'), 'utf8')],
};

const MIGRATION_075: Migration = {
  name: '075_homecare_rls_policies',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '075_homecare_rls_policies.sql'), 'utf8')],
};

const MIGRATION_076: Migration = {
  name: '076_visit_care_plan_link',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '076_visit_care_plan_link.sql'), 'utf8')],
};

const MIGRATION_077: Migration = {
  name: '077_package_draft_fields_nullable',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '077_package_draft_fields_nullable.sql'), 'utf8')],
};

const MIGRATION_078: Migration = {
  name: '078_family_portal_messages',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '078_family_portal_messages.sql'), 'utf8')],
};

const MIGRATION_079: Migration = {
  name: '079_two_person_calls_and_mobile_expansion',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '079_two_person_calls_and_mobile_expansion.sql'), 'utf8')],
};

const MIGRATION_080: Migration = {
  name: '080_visit_swaps_and_notifications',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '080_visit_swaps_and_notifications.sql'), 'utf8')],
};

const MIGRATION_081: Migration = {
  name: '081_add_target_visit_id',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '081_add_target_visit_id.sql'), 'utf8')],
};

const MIGRATION_082: Migration = {
  name: '082_org_location_threshold',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '082_org_location_threshold.sql'), 'utf8')],
};

const MIGRATION_083: Migration = {
  name: '083_org_require_photo',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '083_org_require_photo.sql'), 'utf8')],
};

const MIGRATION_084: Migration = {
  name: '084_org_chat_messages',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '084_org_chat_messages.sql'), 'utf8')],
};

const MIGRATION_085: Migration = {
  name: '085_visit_tasks',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '085_visit_tasks.sql'), 'utf8')],
};

const MIGRATION_086: Migration = {
  name: '086_org_default_rates',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '086_org_default_rates.sql'), 'utf8')],
};

const MIGRATION_087: Migration = {
  name: '087_ride_sharing',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '087_ride_sharing.sql'), 'utf8')],
};

const MIGRATION_089: Migration = {
  name: '089_org_chat_reactions',
  strict: false,
...
const MIGRATION_094: Migration = {
  name: '094_org_emergency_contacts',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '094_org_emergency_contacts.sql'), 'utf8')],
};

// The public demo sandbox and its read-only guard were removed, which left the
// is_demo flag without a reader, so the flag itself goes too.
const MIGRATION_095: Migration = {
  name: '095_drop_demo_account_support',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '095_drop_demo_account_support.sql'), 'utf8')],
};

const MIGRATION_096: Migration = {
  name: '096_chat_attachments',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '096_chat_attachments.sql'), 'utf8')],
};

const MIGRATION_097: Migration = {
  name: '097_homecare_digest_preferences',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '097_homecare_digest_preferences.sql'), 'utf8')],
};

const MIGRATION_098: Migration = {
  name: '098_dated_staff_availability',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '098_dated_staff_availability.sql'), 'utf8')],
};

const MIGRATION_099: Migration = {
  name: '099_homecare_digest_timezones',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '099_homecare_digest_timezones.sql'), 'utf8')],
};

const MIGRATION_100: Migration = {
  name: '100_chat_message_delivery',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '100_chat_message_delivery.sql'), 'utf8')],
};
...
const MIGRATION_101: Migration = {
  name: '101_org_care_capabilities',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '101_org_care_capabilities.sql'), 'utf8')],
};

const MIGRATION_104: Migration = {
  name: '104_contact_submissions',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '104_contact_submissions.sql'), 'utf8')],
};

const MIGRATION_105: Migration = {
  name: '105_sales_pipeline_consent',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '105_sales_pipeline_consent.sql'), 'utf8')],
};

const MIGRATION_106: Migration = {
  name: '106_homecare_rate_profiles',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '106_homecare_rate_profiles.sql'), 'utf8')],
};

const MIGRATION_107: Migration = {
  name: '107_homecare_rate_profile_history',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '107_homecare_rate_profile_history.sql'), 'utf8')],
};

const MIGRATION_108: Migration = {
  name: '108_homecare_payroll_reconciliation_workflow',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '108_homecare_payroll_reconciliation_workflow.sql'), 'utf8')],
};

const MIGRATION_109: Migration = {
  name: '109_homecare_payroll_exported_default',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '109_homecare_payroll_exported_default.sql'), 'utf8')],
};

const MIGRATION_110: Migration = {
  name: '110_homecare_client_billing_delivery',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '110_homecare_client_billing_delivery.sql'), 'utf8')],
};

const MIGRATION_111: Migration = {
  name: '111_homecare_client_invoice_recipients',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '111_homecare_client_invoice_recipients.sql'), 'utf8')],
};

const MIGRATION_112: Migration = {
  name: '112_homecare_timesheet_rate_audit',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '112_homecare_timesheet_rate_audit.sql'), 'utf8')],
};

const MIGRATION_113: Migration = {
  name: '113_domiciliary_subscription_sales_led',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '113_domiciliary_subscription_sales_led.sql'), 'utf8')],
};

const MIGRATION_114: Migration = {
  name: '114_domiciliary_contract_pricing',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '114_domiciliary_contract_pricing.sql'), 'utf8')],
};

const MIGRATION_115: Migration = {
  name: '115_domiciliary_contract_acceptance',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '115_domiciliary_contract_acceptance.sql'), 'utf8')],
};

const MIGRATION_116: Migration = {
  name: '116_domiciliary_contract_audit_rls',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '116_domiciliary_billing_audit_rls.sql'), 'utf8')],
};

const MIGRATION_117: Migration = {
  name: '117_domiciliary_active_contract_amount',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '117_domiciliary_active_contract_amount.sql'), 'utf8')],
};

const MIGRATION_118: Migration = {
  name: '118_domiciliary_subscription_generation',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '118_domiciliary_subscription_generation.sql'), 'utf8')],
};

const MIGRATION_119: Migration = {
  name: '119_homecare_invoice_delivery_tracking',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '119_homecare_invoice_delivery_tracking.sql'), 'utf8')],
};

const MIGRATION_120: Migration = {
  name: '120_timesheets_auto_paid',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '120_timesheets_auto_paid.sql'), 'utf8')],
};

const MIGRATION_121: Migration = {
  name: '121_homecare_timesheet_staff_index',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '121_homecare_timesheet_staff_index.sql'), 'utf8')],
};

const MIGRATION_122: Migration = {
  name: '122_chat_message_file_metadata',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '122_chat_message_file_metadata.sql'), 'utf8')],
};

const MIGRATION_123: Migration = {
  name: '123_staff_supervisions',
  strict: true,
  statements: [fs.readFileSync(path.join(__dirname, 'migrations', '123_staff_supervisions.sql'), 'utf8')],
};
