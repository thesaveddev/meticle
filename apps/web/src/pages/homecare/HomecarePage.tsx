...
export default function HomecarePage() {
  ...
  return <Box className="homecare-page">
    <header className="homecare-header">
      ...
      <Stack direction="row" spacing={1} alignItems="center" flexWrap="wrap"><Chip icon={<HomeWorkIcon />} label={isManager ? `${packages.length} care packages` : `${todayVisits.length} visits today`} className="homecare-chip" />{isManager && <Button size="small" variant="outlined" onClick={() => window.location.href = '/people'} sx={{ textTransform: 'none', borderColor: '#E5E7EB', color: '#374151' }}>Add client</Button>}{isManager && <Button size="small" variant="outlined" onClick={() => window.location.href = '/mileage'} sx={{ textTransform: 'none', borderColor: '#E5E7EB', color: '#374151' }}>Mileage policy</Button>}{isManager && <Button size="small" variant="contained" className="homecare-action" onClick={() => setPackageDialogOpen(true)} disabled={!canCreatePackage}>New package</Button>}</Stack>
    </header>
...
function PackageList({ packages, onMessage }: any) {
  ...
  const [plan, setPlan] = useState({ visit_type: 'routine', label: 'Routine call', days_of_week: [1, 2, 3, 4, 5], start_time: '09:00', duration_minutes: 30, travel_buffer_minutes: 15, default_staff_id: '', default_tasks: [] as { label: string; sort_order: number }[] })
  ...
  const createPlan = async () => {
    ...
    setSaving(true)
    try {
      const created = await api.post(`/homecare/packages/${selectedPackage.id}/visit-plans`, { ...plan, duration_minutes: Number(plan.duration_minutes), travel_buffer_minutes: Number(plan.travel_buffer_minutes), default_staff_id: plan.default_staff_id || null, default_tasks: plan.default_tasks })
