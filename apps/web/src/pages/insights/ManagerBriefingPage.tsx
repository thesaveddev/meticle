    ...
    <PageContainer>
      ...
      {briefing && (
        <Stack spacing={2.5}>
          ...
          {briefing.attention && briefing.attention.length > 0 && <Paper sx={{ p: { xs: 2.5, md: 3 }, border: '1px solid #FDE68A' }}><Stack direction="row" spacing={1} alignItems="center" sx={{ mb: 2 }}><WarningAmber sx={{ color: '#B45309' }} /><Typography variant="h6" sx={{ fontWeight: 800 }}>Needs attention</Typography></Stack><Stack spacing={1.5}>{briefing.attention.map((item, index) => <Box key={`${item.source_id}-${index}`} sx={{ p: 2, bgcolor: item.priority === 'high' ? '#FEF2F2' : '#FFFBEB', border: `1px solid ${item.priority === 'high' ? '#FECACA' : '#FDE68A'}`, borderRadius: 1.5 }}><Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" spacing={1}><Typography sx={{ fontWeight: 800 }}>{item.title}</Typography><Chip label={item.priority} size="small" color={item.priority === 'high' ? 'error' : item.priority === 'medium' ? 'warning' : 'default'} /></Stack><Typography variant="body2" color="text.secondary" sx={{ mt: .75 }}>{item.detail}</Typography><SourceRef type={item.source_type} id={item.source_id} /></Box>)}</Stack></Paper>}
