...
type Result = { headline: string; summary: string; items?: Array<{ title: string; detail: string; priority: string; source_type: string; source_id: string }>; suggested_follow_up?: string[]; limitations?: string[]; generated_by_ai?: boolean }
...
export default function IntelligencePage() {
  ...
  const run = async () => {
    ...
    try {
      const payload: Record<string, string> = { from, to }
      if (question.trim()) payload.question = question.trim()
      if (selected === 'care-summary' && personId.trim()) payload.personId = personId.trim()
  ...
  return <PageContainer>
    <Stack spacing={2.5}>
      ...
      <Paper sx={{ p: { xs: 2, md: 3 }, border: '1px solid', borderColor: 'grey.200' }}>
        ...
        {selected === 'care-summary' && <TextField label="Person ID" size="small" value={personId} onChange={e => setPersonId(e.target.value)} helperText="Open a person record and use its ID for a source-linked summary." sx={{ mt: 2, maxWidth: 520 }} />}
      </Paper>
