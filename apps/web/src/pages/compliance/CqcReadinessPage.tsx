...
`
  ...
  const buildAiPrintHTML = () => {
    ...
    if (!content) return ''
    return `<!DOCTYPE html>
<html><head><title>AI Compliance Gap Analysis - Meticle</title>
<style>
  body { font-family: Arial, sans-serif; padding: 24px; color: #111; max-width: 800px; margin: 0 auto; }
  @media print { body { padding: 16px; } }
</style></head><body>${content}<div class="footer" style="margin-top:24px;padding-top:12px;border-top:1px solid #D1D5DB;font-size:11px;color:#9CA3AF">Meticle AI Analysis &bull; ${new Date().toLocaleString()}</div></body></html>`
  }
  ...
  const buildReportHTML = () => {
    ...
    return `<!DOCTYPE html><html><head><title>Meticle ${frameworkLabel} Readiness Report</title>
      <style>
